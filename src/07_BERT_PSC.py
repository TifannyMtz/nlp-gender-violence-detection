...
def parse_args():
    ...
    p.add_argument("--train", default="data/raw/df_balanceado_PSC_Trans.csv")
    ...
    p.add_argument("--out_model", default="models/bert_psc_Final")
    p.add_argument("--out_results", default="outputs/bert_psc_Final")
...
def main():
    ...
    # RENOMBRAR COLUMNAS
    # ==================================================
    train_df = train_df.rename(columns={"tweet":"text", "clase":"label"})
    val_df = val_df.rename(columns={"tweet":"text", "clase":"label"})
    test_df = test_df.rename(columns={"tweet":"text", "clase":"label"})
    ...
    plt.title("Confusion Matrix - BERT PSC")
    plt.xlabel("Predicho")
    plt.ylabel("Real")
    plt.savefig(f"{args.out_results}/confusion_matrix.png")
    plt.close()
